...
def generate_knn_recommendations(recipeName, df, knn_model, n_neighbors=10):
    #item_index = df[df['item_id'] == item_id].index[0]
    item_index = np.where(info.index == recipeName)[0][0]
...
if st.button('Show Recommendation'):
    recommendations = generate_knn_recommendations(recipeName, info, model)
